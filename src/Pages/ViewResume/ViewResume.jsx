...
const ViewResume = () => {
  ...
  const formatExperienceDetails = (details) => {
    return details.split('.').map((sentence, index) => {
      ...
      return trimmedSentence ? (
        <li key={index}>{trimmedSentence}{trimmedSentence.endsWith('.') ? '' : '.'}</li>
      ) : null;
    });
  };
  ...
  const sortedExperience = [...profile.profileData?.professionalExperience].sort((a, b) =>
    new Date(b.endDate) - new Date(a.endDate)
  );

  const sortedEducation = [...profile.profileData?.education].sort((a, b) =>
      ...
      <div className="resume-container" id="resume-content">
        ...
        {sortedExperience.map((experience, index) => (
          <div key={index} className="professional-experience">
            <h3 className="experience-heading">
              ...
              <span className="experience-dates">({formatDate(experience.startDate)} - {formatDate(experience.endDate)})</span>
            </h3>
            <p><strong>Project:</strong> {experience.projectName}</p>
            <p><strong>Technologies Used:</strong> {experience.techStack}</p>
            <ul className="bullet-section">
        ...
        {sortedEducation.map((education, index) => (
          <div key={index} className="education">
            ...
            <p>Duration: {formatDate(education.duration.split(' - ')[0])} - {formatDate(education.duration.split(' - ')[1])}</p>
          </div>
        ))}
